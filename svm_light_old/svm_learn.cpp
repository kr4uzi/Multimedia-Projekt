...
#define SIGN(x)       ((x) > (0) ? (1) : (((x) < (0) ? (-1) : (0))))
	...
	void svm_learn_classification(DOC **docs, double *jclass, long int
		...
		/* jclass:      Training labels (y-part, zero if test example for
		transduction) */
		...
		optimization if not NULL. Array must be of size totdoc. */
	{
		...
		if(alpha) {
			...
			if(kernel_cache && (kernel_parm->kernel_type != SVM_LIGHT_LINEAR)) {
		...
		/* model:       Returns learning result (assumed empty before called) */
	{
		...
		/* caching makes no sense for linear kernel */
		if((kernel_parm->kernel_type == SVM_LIGHT_LINEAR) && (*kernel_cache)) {
		...
		/* retrain: selects training mode (1=regular / 2=holdout) */
	{
		...
		for(;retrain && (!terminate);iteration++) {
			...
			if((!retrain) && (inactivenum>0) 
				...
				|| (kernel_parm->kernel_type == SVM_LIGHT_LINEAR))) { 
					if(((verbosity>=1) && (kernel_parm->kernel_type != SVM_LIGHT_LINEAR)) 
						|| (verbosity>=2)) {
							if(verbosity==1) {
								printf("\n");
							}
			...
			if((!retrain) && learn_parm->remove_inconsistent) {
				...
				if(retrain) {
					if(kernel_parm->kernel_type == SVM_LIGHT_LINEAR) { /* reinit shrinking */
						learn_parm->epsilon_crit=2.0;
					} 
				}
				if(verbosity>=1) {
		...
		/* Check KT-conditions */
	{
		...
		if(kernel_parm->kernel_type == SVM_LIGHT_LINEAR) {  /* be optimistic */
		...
		minshrink variables can be removed. */
	{
		...
		if((change>=minshrink) /* shrink only if sufficiently many candidates */
			&& (shrink_state->deactnum<shrink_state->maxhistory)) { /* and enough memory */
				...
				if(kernel_parm->kernel_type != SVM_LIGHT_LINEAR) { /*  non-linear case save alphas */
					a_old=(double *)my_malloc(sizeof(double)*totdoc);
		...
		values for linear kernel! */
	{
		...
		if(kernel_parm->kernel_type == SVM_LIGHT_LINEAR) { /* special linear case */
		...
		if(kernel_parm->kernel_type != SVM_LIGHT_LINEAR) { /* update history for non-linear */
			for(i=0;i<totdoc;i++) {
	...
	{                         /* length of the longest support vector. This is */
		...
		JWORD nullword;          /* origin of the space. */
