...
# include <svm_light_old/kernel.h>           /* this contains a user supplied kernel */
// @TODO: remove
//# include <svm_light/shared/Macro.h>        
     ...
     /* classifies one example */
{
  ...
  if((model->kernel_parm.kernel_type == SVM_LIGHT_LINEAR) && (model->lin_weights))
     ...
     /* calculate the kernel function */
{
  ...
  if(kernel_parm->kernel_type == SVM_LIGHT_GRAM) {  /* use value from explicitly */
     ...
     /* calculate the kernel function between two vectors */
{
  ...
  switch(kernel_parm->kernel_type) {
    case SVM_LIGHT_LINEAR: /* linear */ 
            return(sprod_ss(a,b)); 
    case SVM_LIGHT_POLY:   /* polynomial */
            return(pow(kernel_parm->coef_lin*sprod_ss(a,b)+kernel_parm->coef_const,(double)kernel_parm->poly_degree)); 
    case SVM_LIGHT_RBF:    /* radial basis function */
    ...
    case SVM_LIGHT_SIGMOID:/* sigmoid neural net */
            return(tanh(kernel_parm->coef_lin*sprod_ss(a,b)+kernel_parm->coef_const)); 
    case SVM_LIGHT_CUSTOM: /* custom-kernel supplied in file kernel.h*/
            return(custom_kernel(kernel_parm,a,b)); 
    default: printf("Error: Unknown kernel function\n"); exit(1);
  }
}

SVECTOR *create_svector(JWORD *words,char *userdefined,double factor)
{
  SVECTOR *vec;
  long    fnum,i;
  ...
  vec->words = (JWORD *)my_malloc(sizeof(JWORD)*(fnum));
  for(i=0;i<fnum;i++) { 
      vec->words[i]=words[i];
  }
  vec->twonorm_sq=-1;
...
SVECTOR *create_svector_shallow(JWORD *words,char *userdefined,double factor)
...
SVECTOR *create_svector_n_r(double *nonsparsevec, long maxfeatnum, char *userdefined, double factor, double min_non_zero)
{
  ...
  for(i=1;i<=maxfeatnum;i++)  
    if((nonsparsevec[i]<-min_non_zero) || (nonsparsevec[i]>min_non_zero))
      fnum++;
  vec = (SVECTOR *)my_malloc(sizeof(SVECTOR));
  vec->words = (JWORD *)my_malloc(sizeof(JWORD)*(fnum+1));
  fnum=0;
  for(i=1;i<=maxfeatnum;i++) { 
    if((nonsparsevec[i]<-min_non_zero) || (nonsparsevec[i]>min_non_zero)) {
	...
	SVECTOR is used */
{
    ...
    sum=(JWORD *)my_malloc(sizeof(JWORD)*veclength);
    sumi=sum;
    ai=a->words;
    bj=b->words;
    while (ai->wnum && bj->wnum) {
...
{
  SVECTOR *sum,*f;
  JWORD    empty[2],*ai,*concat,*concati,*concat_read,*concat_write;
  long    length,i;
  double  weight;
  ...
  if(a){
    ...
    concat=(JWORD *)my_malloc(sizeof(JWORD)*(length+1));
    concati=concat;
    for(f=a;f;f=f->next) {
      ai=f->words;
      while (ai->wnum) {
    ...
    qsort(concat,length,sizeof(JWORD),compareup_word);
     ...
     /* compute length of weight vector */
{
  ...
  if(model->kernel_parm.kernel_type != SVM_LIGHT_LINEAR) {
...
void write_model(char *modelfile, MODEL *model)
{
  ...
  if(0 && model->kernel_parm.kernel_type == SVM_LIGHT_LINEAR) {
    if(verbosity>=1) {
...
MODEL *read_model(char *modelfile)
{
  ...
  words = (JWORD *)my_malloc(sizeof(JWORD)*(max_words+10));
		    ...
		    long int *totwords, long int *totdoc)
{
  ...
  words = (JWORD *)my_malloc(sizeof(JWORD)*(max_words_doc+10));
  if(verbosity>=1) {
...
int parse_document(char *line, JWORD *words, double *label,
